...
export default function CourseContentPage({
  ...
  const isModuleCompleted = (moduleId: string) => {
    if (!currentCourseProgress) return false;

    return currentCourseProgress.modules.some(
  ...
  const handleLessonComplete = async (currentLessonId: string) => {
    if (!courseId || isLessonCompleted(currentLessonId)) return;
    try {
      await updateProgress(courseId, currentLessonId);
      // Store will automatically update through subscription
    } catch (error) {
  ...
  const isLessonCompleted = (lessonId: string) => {
    ...
    return currentCourseProgress.modules.some((module) =>
      module.lessons.some(
        (lesson) => lesson.lessonId === lessonId && lesson.completed
      )
    );
  };
  ...
  const getModuleProgress = (moduleId: string): number => {
    try {
      if (!currentCourseProgress?.modules) return 0;

      const module = currentCourseProgress.modules.find(
        (m) => m.moduleId === moduleId
      );

      if (!module) return 0;
      return Math.min(Math.max(module.progress || 0, 0), 100); // Ensure 0-100 range
    } catch (error) {
  ...
  const updateUrl = (lessonId: string) => {
    if (!course) return;

    const courseSlug = encodeURIComponent(
      course.title.replace(/\s+/g, "-").toLowerCase()
    );
    const newUrl = `/courses/my-courses/${course.id}/${courseSlug}/lesson/${lessonId}`;
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      ...
      if (event.state && event.state.lessonId && course) {
        // Find the lesson in our course data
        let found = false;
        course.modules.forEach((module, moduleIndex) => {
          module.lessons.forEach((lesson, lessonIndex) => {
            if (lesson.id === event.state.lessonId) {
              setCurrentModuleIndex(moduleIndex);
              setCurrentLessonIndex(lessonIndex);
                  ...
                  <div className="flex items-center gap-2">
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          {isLessonCompleted(currentLesson.id)
                            ? "Lesson completed - Progress cannot be undone"
                ...
                >
                  ...
                  <div className="container mx-auto p-4 md:p-6">
                    <TabsContent value="content" className="mt-0 border-0 p-0">
                      ...
                      {currentCourseProgress?.overallProgress === 100 && (
                        <div className="mt-8 p-6 border rounded-lg bg-primary/5 text-center">
                          <h3 className="text-xl font-semibold mb-2">
                            Congratulations! 🎉
                          </h3>
                          <p className="text-muted-foreground mb-4">
                            You've completed all lessons in this course. Ready
            ...
            <div className="top-20 space-y-6">
              <Card>
                <div className="p-4 border-b">
                  ...
                  <Progress
                    value={currentCourseProgress?.overallProgress}
                    className="h-2 mb-4"
                  />
